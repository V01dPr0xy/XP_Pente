...
public class ValidityTests
{
	...
	//Trias
	public static bool CheckForTriaTop(int[,] currentState, int x, int y)
	{
		int player = currentState[x, y];
		int length = (int)Math.Sqrt(currentState.Length);
		...
		if (y - 4 <= -1)
		{
			if (y + 1 < length) { if (currentState[x, y + 1] != -1) return false; }
			else { return false; }
		}
		else if (currentState[x, y - 4] != -1)
		{
			if (y + 1 < length) { if (currentState[x, y + 1] != -1) return false; }
			else { return false; }
		}
		...
		if (y - 2 == 0) return false;
		if (y - 3 == 0) return false;
		if (!(currentState[x, y - 1] == player)) return false;
		if (!(currentState[x, y - 2] == player)) return false;
		return true;
	}
	public static bool CheckForTriaBottom(int[,] currentState, int x, int y)
	{
		int player = currentState[x, y];
		int length = (int)Math.Sqrt(currentState.Length);
		if (y == length) return false;
		...
		if (!(currentState[x, y + 1] == player)) return false;
		if (!(currentState[x, y + 2] == player)) return false;
		if (currentState[x, y + 3] == -1) return true;
		return false;
	}
	public static bool CheckForTriaLeft(int[,] currentState, int x, int y)
	{
		int player = currentState[x, y];
		if (x == 0) return false;
		if (x - 1 == 0) return false;
		if (x - 2 == 0) return false;
		if (x - 3 == 0) return false;
		if (!(currentState[x - 1, y] == player)) return false;
		if (!(currentState[x - 2, y] == player)) return false;
		if (currentState[x - 3, y] == -1) return true;
		return false;
	}
	public static bool CheckForTriaRight(int[,] currentState, int x, int y)
	{
		int player = currentState[x, y];
		int length = (int)Math.Sqrt(currentState.Length);
		if (x == length) return false;
	...
	public static bool CheckForTriaTopLeft(int[,] currentState, int x, int y)
	{
		int player = currentState[x, y];
		int length = (int)Math.Sqrt(currentState.Length);
		if (x == 0 || y == 0) return false;
	...
	public static bool CheckForTriaTopRight(int[,] currentState, int x, int y)
	{
		int player = currentState[x, y];
		int length = (int)Math.Sqrt(currentState.Length);
		if (x == length || y == 0) return false;
	...
	public static bool CheckForTrialBottomRight(int[,] currentState, int x, int y)
	{
		int player = currentState[x, y];
		int length = (int)Math.Sqrt(currentState.Length);
		if (x == length || y == length) return false;
	...
	public static bool CheckForTriaBottomLeft(int[,] currentState, int x, int y)
	{
		int player = currentState[x, y];
		int length = (int)Math.Sqrt(currentState.Length);
		if (x == 0 || y == length) return false;
